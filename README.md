...
Custom plugins are supported automatically. When `re_assert: true`, any rules
generated by plugin macros (those starting with `dbt_assertions__`) are also
recomputed in-query and included in the filter logic.
...
You can also extend the package with your own reusable assertion types by
writing plugin macros. Any macro whose name begins with `dbt_assertions__`
(e.g. `dbt_assertions__positive`, `dbt_assertions__is_in`) will be automatically
discovered and executed when a matching helper key (e.g. `__positive__`) appears
in the `assertions:` section of your YAML.

Plugin macros must return a dictionary of assertion IDs mapped to objects with
`description` and `expression` fields, just like built-in helpers. See the built-in
macros for examples.
...
```
These will invoke the macros dbt_assertions__positive and
dbt_assertions__is_in respectively, if defined in your project or in a dependency.

##### Add a Adapter-Specific macro with slight variations
e.g.
```
macros/dbt_assertions__positive.sql
macros/duckdb__dbt_assertions__positive.sql
```
If you run `dbt build` and your active adapter is `DuckDB`, and both macros exist:

`dbt` uses `duckdb__dbt_assertions__positive.sql`

If you switch to `BigQuery` or `Snowflake`:

`dbt` uses `dbt_assertions__positive.sql`

No code changes needed.

> **NOTE:** you could build `BigQuery` or `Snowflake` versions with a prefix as well. 
